...
int main() {
    ...
    try{
        ...
        PROCESS_INFORMATION pi;//тут будут дескрипторы и идентификаторы процесса и потока

        std::string cmd = "Creator.exe " + binFile + " " + std::to_string(records);

        if (!CreateProcessA(NULL, (LPSTR)cmd.c_str(), NULL, NULL, FALSE,
            CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
            throw std::runtime_error("Failed to start Creator (Error: " + std::to_string(GetLastError()) + ")");
        }
        ...
        if (!CreateProcessA(NULL, (LPSTR)cmd.c_str(), NULL, NULL, FALSE,
            CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
            std::cerr << "Failed to start Creator";
        }
